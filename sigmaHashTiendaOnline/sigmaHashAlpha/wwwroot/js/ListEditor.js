	...
	//function compact() {
		...
		setTimeout(function () {
			...
			for (var i = 1; i <= 6; i++) {
				...
				let input = '#addpic' + i.toString();
				let box = '#gal' + i.toString();
				$(document).on('change', input, function () {
					let target = $(box);

					Previewer(this, target);
					$(input).parent().find('label').css('pointer-events','none');
					$(input).parent().find('label').css('border-color', 'blue');
			...
			//		$(p).html("");
			//		$(k).parent().find('label').attr({ 'for': input });
